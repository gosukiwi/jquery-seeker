...
this._checkForAutocomplete=function(){var a,d,e=c.val();for(a=0;a<c.source.length;a++)if(d=c.source[a][c.settings.seekField].toString().toLowerCase(),0===d.indexOf(e)){a=c.source[a];c.setSelectedItem(a);c.settings.peerSeeker&&c.settings.peerSeeker.setSelectedItem(a);b.hide();break}};m=this.id;c=this;if(this.settings.url){var q=this.settings.url+"."+this.settings.method;__jquerySeekerPluginAjaxCache[q]?(d=__jquerySeekerPluginAjaxCache[q],c.settings.source=d,c.source=d):i.ajax({type:this.settings.method,
contentType:"application/json; charset=utf-8",url:this.settings.url,dataType:"json",async:!1,success:function(b){c.settings.source=b;c.source=b;__jquerySeekerPluginAjaxCache[q]=b}})}else for(a=0;a<this.settings.source.length;a++)this.source.push(this.settings.source[a]);null!==this.settings.order&&(a=this.settings.orderBy||this.settings.seekField,p="DESC"===this.settings.order,this.source.sort(function(b,c){return b[a]===c[a]?0:p&&b[a]>c[a]||!p&&b[a]<c[a]?-1:1}));this.addClass("seeker-text");this.wrap('<div class="seeker-container" />');
this.after('<div class="seeker-table-container"><table class="seeker-outter" id="'+m+'-table"></table></div>');this.css("width",this.settings.width);b=i("#"+m+"-table");d=b.parent();this.settings.dropDownSameWidth&&b.width(this.width());this._buildTable(this.source);o=l=f=h=!1;this.after('<input type="button" value="" id="'+m+'-button" class="seeker-button" />');e=i("#"+m+"-button");this.bind("mouseenter",function(){f=true;e.show()});this.bind("mouseleave",function(){f=false;h||e.hide();l||b.hide()});
this.bind("focus",function(){h=true;e.show()});this.bind("blur",function(){h=false;f||e.hide();if(o){o=false;b.hide()}});e.bind("mouseenter",function(){f=true;e.show()});e.bind("mouseleave",function(){f=false;h||e.hide()});e.bind("click",function(){if(c.filteredSource.length>0){c.filteredSource=[];c._buildTable(c.source)}if(b.css("display")==="none"){b.show();c._updateScroll(b,c.selectedIndex,c.source.length)}else b.hide()});d.bind("mouseenter",function(){l=true;b.show()});d.bind("mouseleave",function(){l=
false;!h&&!f&&b.hide()});this.bind("keyup",function(a){if(a.keyCode===40||a.keyCode===38){a=c.selectedIndex+(a.keyCode===40?1:-1);a=(new j).clamp(a,0,c.source.length-1);c.setSelectedItem(c.source[a]);b.show();c._updateScroll(b,c.selectedIndex,c.source.length);c.settings.peerSeeker&&c.settings.peerSeeker.setSelectedItem(c.source[a])}else if(a.keyCode===13)b.hide();else{c._filterSource(i(this).val());b.show();if(c.settings.autocompleteInterval>0){k&&clearInterval(k);k=setInterval(c._checkForAutocomplete,
c.settings.autocompleteInterval)}}});return this}})(jQuery);